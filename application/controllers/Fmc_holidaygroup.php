...
class Fmc_holidaygroup extends MY_Controller {
	...
	public function index($tab="")
	{
		...
		$data['current_tab'] = ($tab != "") ? $tab : 'holiday-group';
	...
	{		
		if($this->input->post()){
	        ...
	        $is_inserted = $this->holidays_group_model->insert($insert_data);

	        if($is_inserted){
	        	if($this->input->post('holidays')){
	        		$holiday_group_array = array();
		        	foreach ($this->input->post('holidays') as $value) {
		        		$holiday_group_array[] = array(
		        			'holiday_id' => $value,
		        			'holiday_group_id' => $is_inserted
		        		);
		        	}
	        		$this->holidays_model->insert_holiday_groups_common($holiday_group_array);
	        	}

	        	$this->session->set_flashdata('flashSuccess', 'Holiday Group has been inserted successfully.');
	        }else{
	        	$this->session->set_flashdata('flashError', 'Holiday Group has not been inserted successfully.');
	        }	        
	        redirect('/fmc-holiday-group');
	...
	public function update($id = "")
	{
		...
		if($this->input->post()){
	        ...
	        $is_updated = $this->holidays_group_model->update($updateData);
	        ...
	        if($is_updated){
	        	if($this->input->post('holidays')){
		        	...
		        	foreach ($this->input->post('holidays') as $value) {
		        		$holiday_group_array[] = array(
		        			'holiday_id' => $value,
		        			'holiday_group_id' => $this->input->post('id')
		        		);
		        	}
	        		$this->holidays_model->insert_holiday_groups_common($holiday_group_array);
	        	}

	        	$this->session->set_flashdata('flashSuccess', 'Holiday Group has been updated successfully.');
	        }else{
	        	$this->session->set_flashdata('flashError', 'Holiday Group has not been updated successfully.');
	        }
	        redirect('/fmc-holiday-group');
	    }else{
	    	$this->session->set_flashdata('flashError', 'Invalid request');
	...
	function get(){
		if($this->input->post()){
			...
			$department_data = $this->holidays_group_model->get_details($this->input->post('id'));

			if($department_data){
				$holiday_group_data = $this->holidays_model->get_holiday_groups_common("",$this->input->post('id'));

				if($holiday_group_data){
					$department_data['holidays'] = $holiday_group_data;
				}else{
					$department_data['holidays'] = array();
				}
	...
	public function delete($id = "", $status = ""){
        ...
        $deleted = $this->holidays_group_model->delete($id, $status);

        if($deleted){
        	$this->session->set_flashdata('flashSuccess', 'Holiday Group has been deleted successfully.');
            echo '1';exit;
        }else{
        	$this->session->set_flashdata('flashSuccess', 'Holiday Group has not been deleted successfully.'); 
            echo '0';exit;
        }
	}	
